...
Open `book-list.component.html` file in `app\book\book-list` folder and replace the content as below:
...
Open `book-list.component.html` in `app\book\book-list` folder and replace `<ng-template #abpBody> </ng-template>`  with the following code part:
...
Open `book.module.ts` file in `app\books` folder and replace the content as below:
